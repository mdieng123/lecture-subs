...
export default function ClipsScreen() {
  ...
  const { clips, detecting, error, toggleClip, selectAll, deselectAll } = useClipsStore()
  ...
  if (error) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => setScreen('editor')} className="text-sm text-[hsl(215,15%,55%)] hover:text-white">
          ← Back to Editor
        </button>
      </div>
    )
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-[hsl(220,15%,22%)] bg-[hsl(222,20%,12%)] flex-shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={() => setScreen('editor')} className="text-[hsl(215,15%,50%)] hover:text-white text-sm">
            ← Back
          </button>
...
function ClipCueRow({ cue, isLast, onUpdate, onMerge, onDelete }: {
  cue: Cue
  isLast: boolean
  ...
  return (
    <div className="flex flex-col gap-0.5 p-1.5 rounded bg-[hsl(222,20%,16%)] border border-[hsl(220,15%,22%)]">
      <div className="flex items-center justify-between">
        <span className="text-[10px] text-[hsl(210,80%,60%)] font-mono">{fmt(cue.startSeconds)} → {fmt(cue.endSeconds)}</span>
        <div className="flex gap-1">
          {!isLast && (
            <button onClick={onMerge} className="text-[9px] px-1 py-0.5 rounded border border-[hsl(220,15%,28%)] text-[hsl(215,15%,50%)] hover:text-white">merge↓</button>
          )}
          <button onClick={onDelete} className="text-[9px] px-1 py-0.5 rounded border border-[hsl(220,15%,28%)] text-[hsl(215,15%,50%)] hover:text-red-400">×</button>
        </div>
      </div>
      <textarea
        value={cue.english}
